...
#include <AccelStepper.h>
  ...
  AccelStepper lidarStepper(FULLSTEP, stepPin[0], stepPin[1], stepPin[2], stepPin[3]);
...
void setup(void){
  ...
  #ifdef LIDAR
   lidarStepper.setMaxSpeed(250.0);
   lidarStepper.setAcceleration(100.0);
  #endif
...
//    stepTo(scanMin);
  ...
  void handleStepperLeft()
  {
    ...
    lidarStepper.runToNewPosition(lidarStepper.currentPosition()+10);
  }
  ...
  void handleStepperHome()
  {
    ...
    lidarStepper.runToNewPosition(0);
    stepperOff();
  }
  ...
  void handleScanStepPlus()
  {
    ...
    lidarStepper.runToNewPosition(lidarStepper.currentPosition()+20);
  }
...
void handleData() {
  ...
  #ifdef LIDAR 
    response["Angle"] = roundf(lidarStepper.currentPosition()*0.17);
    response["HasServo"] = true;
    response["Step"] = lidarStepper.currentPosition();
    response["ScanStep"] = 17;
    response["ManualStep"] = 84;
  ...
  Serial.println(lidarStepper.currentPosition());
...
//     if ( lidarStepper.distanceToGo() != 0) {   // only move if needed
//      if ( !stepperPwr ) stepperOn(); // wake stepper as needed
//      #ifdef SERVO_INVERT
//        lidarStepper.move(-(target-currentStep));
//      #else
//        lidarStepper.move((target-currentStep));
//      #endif
//      currentStep = target;
...
void loop(void){
  ...
  #ifdef LIDAR
    lidarStepper.run();
     if (lidarStepper.distanceToGo() == 0 && scanControl != 0)
      lidarStepper.moveTo(-lidarStepper.currentPosition());
  #endif
}
